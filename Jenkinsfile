node('local') {
    try {
        ...
        stage('Build & Test') {
            sh "#!/bin/bash\n" +
                    "set -e -v -o pipefail\n" +
                    "${tool name: 'sbt 1.0.4', type: 'org.jvnet.hudson.plugins.SbtPluginBuilder$SbtInstallation'}/bin/sbt -ivy /tmp/.ivy2 -no-colors clean evicted biopetTest 'set test in assembly := {}' assembly | tee sbt.log"
